...
```bash
# 1. Create venv + install (editable + dev deps)
make install

# 2. Download data (cached as parquet after first run)
make data

# 3. Train the canonical MLP + evaluate
make train
make eval

# 4. Run the full diagnostic pipeline (sweep, walk-forward, residuals,
#    per-ticker breakdown, HTML report)
make report

# Shortcut: train + eval + full report in one go
make all
...
The full diagnostic HTML lands at `results/comparisons/report.html` and is
self-contained (PNGs base64-embedded).
...
├── src/                     # flat module layout (declared in pyproject.toml)
│   ├── paths.py             # centralised RAW/RESULTS/FIGURES directories
│   ├── data_loader.py       # download + parquet cache + min-bars fallback
...
├── results/                 # .gitignore'd (regenerated by `make all`)
│   └── README.md
